...
Before release, verify that `curbshade-fetch-osm --help` and `curbshade-validate-osw --help` are present in the built wheel, that the `core`, `osm`, and `osw` dependency-audit jobs are green, and that OSW tests validate against an explicit Draft 7 schema. A schema-breaking network change requires a new `schema_version`, not a silent edit to `curbshade-network/1`.
